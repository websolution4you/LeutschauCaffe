...
(function () {
  ...
  const container = document.querySelector('.events-classic-wrap');
  const adminContainer = document.createElement('div');
  adminContainer.id = 'admin-events-controls';
  ...
  function setupAdminUI() {
    ...
    adminContainer.innerHTML = `
      <div style="text-align: left; background: #fff; padding: 25px; border-radius: 12px; box-shadow: 0 4px 20px rgba(0,0,0,0.1); border: 1px solid #eee; margin-bottom: 30px;">
        <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 25px; flex-wrap: wrap; gap: 15px;">
            <h4 style="margin: 0; color: #a3734a; font-family: 'Roboto Slab', serif; font-weight: 700; min-width: 200px;">Administrácia Eventov</h4>
            <div style="display: flex; gap: 10px; flex-wrap: wrap;">
                <button id="admin-save-all-btn" style="background: #28a745; color: white; border: none; padding: 10px 15px; border-radius: 6px; cursor: pointer; font-size: 0.9rem; font-weight: bold; white-space: nowrap;">Uložiť zmeny</button>
        ...
        <div style="background: #fcfcfc; padding: 20px; border-radius: 10px; border: 1px solid #eaeaea;">
            <p style="font-size: 0.9rem; margin-bottom: 15px; color: #444; font-weight: bold;">Pridať nový event:</p>
            <div style="display: flex; flex-direction: column; gap: 15px;">
              ...
              <input type="text" id="admin-title-input" placeholder="Názov eventu" style="padding: 10px 15px; border: 1px solid #ddd; border-radius: 6px; font-size: 0.9rem;">
              <textarea id="admin-desc-input" placeholder="Popis eventu" style="padding: 10px 15px; border: 1px solid #ddd; border-radius: 6px; font-size: 0.9rem; min-height: 100px;"></textarea>
              <button id="admin-upload-btn" style="background: #007bff; color: white; border: none; padding: 11px 30px; border-radius: 6px; cursor: pointer; font-weight: bold; display: none;">Pridať event</button>
            </div>
    ...
    document.getElementById('admin-save-all-btn').onclick = async () => {
        ...
        const items = Array.from(document.querySelectorAll('.event-item-wrap'));
        ...
        for (let i = 0; i < updates.length; i++) {
            await supabase.from('events_items').update({ 
    ...
    uploadBtn.onclick = async () => {
      ...
      if (!title) { alert('Prosím zadajte názov eventu'); return; }
      ...
      try {
          const { data, error: signErr } = await supabase.functions.invoke('s3-manager', {
              body: { action: 'sign', fileName: file.name, contentType: file.type, folder: 'events' }
          });
          ...
          const { error: dbErr } = await supabase.from('events_items').insert({
              s3_key: data.key, 
              title: title,
              description: desc,
              mime_type: file.type, 
          ...
          status.innerText = `Event úspešne pridaný!`;
  ...
  async function loadEvents(isAdmin) {
    ...
    if (events.length === 0 && !isAdmin) return;
    ...
    events.forEach((item, index) => {
        ...
        article.className = 'post-classic event-item-wrap';
        ...
        if (isAdmin) {
            ...
            <div style="margin-top: 15px; background: #fff; padding: 15px; border-radius: 8px; border: 1px solid #ddd; margin-bottom: 30px;">
                <div style="display: flex; flex-direction: column; gap: 10px; margin-bottom: 12px;">
                    <div style="display: flex; gap: 10px;">
                        <div style="flex-grow: 1;">
                            <label style="font-size: 0.7rem; color: #777; display: block; margin-bottom: 4px; font-weight: bold;">Názov Eventu</label>
                    ...
                    <div>
                        <label style="font-size: 0.7rem; color: #777; display: block; margin-bottom: 4px; font-weight: bold;">POPIS</label>
                ...
                <div style="text-align: right;">
                    <button class="admin-del-btn" data-id="${item.id}" data-key="${item.s3_key}" style="background: #dc3545; color: white; border: none; padding: 8px 15px; border-radius: 6px; cursor: pointer;">Zmazať event</button>
                </div>
            </div>
            `;
        }
        ...
        article.innerHTML = `
            ...
            <ul class="list-inline list-inline-vertical-line">
              <li>${dateStr}</li>
              <li><span>pridané: Leutschau Café</span></li>
            </ul>
            ${adminControls}
        `;
    ...
    if (isAdmin) {
        document.querySelectorAll('.admin-del-btn').forEach(btn => {
            btn.onclick = async () => {
                ...
                await supabase.from('events_items').delete().eq('id', btn.dataset.id);
                location.reload();
            };
        });
    }
